...
const $contributorModalSave = $('#contribute .save');
...
function renderContributors() {
  let $contributorListItems = board.users.map(function(user) {
    ...
    let $contributorDeletButton = $('<button class="danger">Remove</button>')
      .data(user)
      .on('click', handleContributorDelete);

    $contributorListItem.append($contributorSpan, $contributorDeletButton);
  ...
  $('.list > ul').each(function(index, element) {
    Sortable.create(element, {
      ...
      onEnd: function(event) {
        ...
        if (newPosition === position && newListId === list_id) {
          return;
        }
...
$contributorModalSave.on('click', handleContributorSave);
